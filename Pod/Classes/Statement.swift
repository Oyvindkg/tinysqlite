...
// MARK: - Internal methods
    ...
    internal func prepareForDatabase(_ databaseHandle: OpaquePointer) throws {
        let pointer = UnsafeMutablePointer<OpaquePointer?>(handle)
        
        try SQLiteResultHandler.verifyResultCode(sqlite3_prepare_v2(databaseHandle, query, -1, pointer, nil), forHandle: handle)
    }
